...
func Test_AssetIDValidateBasic(t *testing.T) {
	tests := []struct {
		...
		{"-ve", &AssetID{&HashID{[]byte("test")}}, errorConstants.IncorrectFormat.Wrapf("invalid hashID length")},
	}
...
func Test_AssetIDFromString(t *testing.T) {
	tests := []struct {
		...
		{"+ve", "test", &AssetID{&HashID{[]byte{}}}, errorConstants.IncorrectFormat.Wrapf("invalid hashID length")},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
